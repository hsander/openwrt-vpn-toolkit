...
> **⚠ Статус: DRAFT.** Все скрипты прошли syntax-чек и mock-тесты против недоступного хоста, но **end-to-end против реального роутера ещё не прогнан**. Перед первым live-запуском см. `SKILL.md §Scope V1` и обязательно сделай ручной snapshot роутера (например, `cp -a /etc /tmp/etc-backup`). Когда e2e отработает — баннер уберём.
