#!/usr/bin/env bash
# Reconcile netifd/firewall after final removal of 192.168.1.0/24.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      exit 0 ;;
    *) echo "reconcile-home-lan-final-runtime: unknown arg: $1" >&2; exit 13 ;;
  esac
done

[[ "$rollback_snapshot" =~ ^snap-[0-9]{8}T[0-9]{6}Z$ ]] || exit 13
resolve_router_config "$router"

pre_snapshot="$($OPENWRT_SKILL_HOME/bin/backup-now.sh --router "$ROUTER_ALIAS" \
  --label 'before final netifd LAN99 reconciliation' --quiet)"
[[ -n "$pre_snapshot" ]] || exit 13
...
set -eu
rollback_snapshot="$1"
timer='/etc/vpn-kit/rollback.d/lan99-final-reconcile.timer'
...
mkdir -p /usr/local/sbin /etc/vpn-kit/rollback.d
...
deadline=$(( $(date +%s) + 300 ))
jq -n \
  --arg step_id lan99-final-reconcile \
...
printf '%s\npre_reconcile_snapshot=%s\n' "$arm_result" "$pre_snapshot"

ssh_run "setsid sh -c 'ubus call network reload >/etc/vpn-kit/lan99-final-reconcile.log 2>&1; ip neigh flush to 192.168.1.0/24 dev br-lan >/dev/null 2>&1 || true' </dev/null &" \
  >/dev/null 2>&1 || true
...
! ip -4 route show | grep -q '^192\.168\.1\.0/24 '
ubus call network.interface.lan status \
  | jq -e 'any(."ipv4-address"[]?; .address == "192.168.99.1")
    and (any(."ipv4-address"[]?; .address == "192.168.1.1") | not)' >/dev/null
grep -hE '^dhcp-range=set:lan,192\.168\.99\.200,192\.168\.99\.249,255\.255\.255\.0,12h$' \
  /var/etc/dnsmasq.conf.* >/dev/null
! nft list ruleset 2>/dev/null | grep -q '192\.168\.1\.'
! uci -q show network | grep -q '192\.168\.1\.'
! uci -q show firewall | grep -q '192\.168\.1\.'
! jq -e '.. | strings | select(contains("192.168.1."))' /etc/sing-box/config.json >/dev/null
! grep -q '192\.168\.1\.' /etc/init.d/sing-box-tproxy
...
if [[ "$validated" != 1 ]]; then
  echo 'final reconcile: validation failed; autonomous rollback remains armed' >&2
  exit 20
fi

ssh_run_remote <<'SH'
set -eu
rm -f /etc/vpn-kit/rollback.d/lan99-final-reconcile.timer
