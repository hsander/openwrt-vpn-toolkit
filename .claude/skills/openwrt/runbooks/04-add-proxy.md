...
3. **Listen IP (опционально).** Default: `192.168.1.1`. Спрашивай только если пользователь явно сказал что у него LAN другой подсети или нужен `0.0.0.0`. Никогда не предлагай `0.0.0.0` сам — это безопасно только если уверен в WAN-firewall.
